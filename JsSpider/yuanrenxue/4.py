...
def main():
    ...
    # # print(resp)
    # info = resp['info']
    # # print(info) 9284093138 0277464438   1266530368
    # img_urls = re.findall('<td><(.*?)>',info,re.S)
    # print(img_urls)
    # # value = re.findall('base64,(.*?)"',str(img_urls),re.S)
    # # # print(value)
    ...
    for i in range(1,6):
        ...
        # print(j_key)
        img_list = re.findall('(?<=<td>).+?(?=</td>)', response['info'])
        for img_str in img_list:
            imgs = re.findall('(?<=<img).+?(?=>)', img_str)
            out = ['', '', '', '', '']
            i = 0
            for img in imgs:
                # print(re.findall('(?<=style="left:).+?(?=px")', img)[0])
                number = re.findall('(?<=data:image/png;base64,).+?(?=")', img)[0]
                imgclass = re.findall('(?<=img_number ).+?(?=")', img)[0]
                imgstyle = int(round(float(re.findall('(?<=style="left:).+?(?=px")', img)[0])) / 11)
