...
def split_picture(imagepath):
    ...
    for i,item in enumerate(sorted_chars_list):
        x, y, w, h = item
        cv2.imwrite('C://CodeTime/xxxx/ver/%d.jpg'%(i+1), thresh1[y:y+h, x:x+w])
...
def resplit_with_parts(imagepath, parts):
    ...
    for i in range(parts):
        cv2.imwrite('C://CodeTime/xxxx/ver/%s.jpg'%(file_name+'-'+str(i)), \
...
def predict(VerifyCodePath):
    ...
    if not files:
        ...
        # 模型保存地址
        mp = 'C://CodeTime/xxxx/verifycode_Keras.h5'
        # 载入模型
...
def main():
    ...
    dir = 'C://CodeTime/xxxx/VerifyCode/'
    # dir = 'C://CodeTime/img/'
    correct = 0
